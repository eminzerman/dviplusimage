function dviPlusIm = dviplusimage(fileFolderName, debugFlag, testPhrase, outFolder)
...
for imNo = fileIndices
    ...
    if ~isempty(find(hdrIm<=0,1))
        hdrIm(hdrIm<=0) = 1e-4;
        warning('Removing negative values from the image');
    end
    ...
    % Scale the LEDs 
    SAFETY_MARGIN = 30;
    scale = imdilate(targetLumSm + SAFETY_MARGIN, ones(25,25))./ledLumSm;
