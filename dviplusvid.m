function dviplusvid(folderName, outFolder, mod, windowsize)
%DVIPLUSVID Creates DVI Plus video for SIM2 HDR47 screen.
%   DVIPLUSVID(folderName) Creates DVI Plus video. The function requires 
%   folderName as input. This input should be a directory.
%
...
if(~exist('outFolder', 'var')), 
    ...
    if ~isdir([folderName filesep outFolder])
        mkdir([folderName filesep outFolder]);
    end
    outFolder = [folderName filesep outFolder];
else
    if ~isdir(outFolder)
        mkdir(outFolder);
    end
...
if ~exist('windowsize', 'var'), windowsize = 7; end
Params.mod = mod;
...
Params.windowSize = max(7, windowsize); % min 7
Params.windowTypeCentered = false;
Params.otherFramesScale = 1;
...
% Find the frame numbers within the window for each frame
Params = findWindowParams(Params, DbContents);
...
for imNo = Params.fileIndices
    ...
    if Params.fileFlag
        [~, Ims.fileName, Ims.fileXt] = fileparts(Params.folderName);
    else
    ...
    [Ims, Params] = desiredBacklightVid(DbContents, Params, Ims, imNo);

    %% Find LED values
    Ims = findLedValues(Params, Ims, imNo);
...
function [Params, DbContents] = determineFileFolder(Params)
    ...
    if ~isempty(strfind(Params.folderName(end-4:end),'.hdr')) || ...
            ~isempty(strfind(Params.folderName(end-4:end),'.exr'))
        % ========== It is an HDR or EXR file ==========
        Params.fileFlag = 1;
        Params.fileIndices = 1;
        DbContents.name = Params.folderName;
    elseif isdir(Params.folderName)
        % ========== It is a folder ==========
        Params.fileFlag = 0;
        % Find folder contents
        countHdrFiles = length(dir([Params.folderName '\*.hdr']));
        countExrFiles = length(dir([Params.folderName '\*.exr']));
        if countHdrFiles == 0 && countExrFiles == 0
            error('No .hdr or .ext files present in specified path!');
        else
            if countHdrFiles > countExrFiles
                % Read HDR
                DbContents = dir([Params.folderName '\*.hdr']);
            else
                %Read EXR
                DbContents = dir([Params.folderName '\*.exr']);
            end
        end
...
function [Ims, rFlag] = readHdrIm(imName, Params, Ims)
    ...
    if Params.fileFlag
        ...
        if strcmp(Ims.fileXt,'.hdr')
            try
                hdrIm = hdrread([Params.folderName filesep imName]); 
            catch
                try
                    hdrIm = hdrimread([Params.folderName filesep imName]);
                catch
                    error('Problem acquiring the HDR image/frame!!');
                end
            end
...
function imLum = readHdrThisIm(imName, Params, Ims)
    if Params.fileFlag
        if strcmp(Ims.fileXt,'.hdr')
            try
                hdrIm = hdrread(imName); 
...
function Ims = desiredBacklight(Params, Ims)
    ...
    targetLum = desiredBacklightBase(Params, Ims.hdrLum);
...
function outIm = desiredBacklightBase(Params, inputLum)
    ...
    targetLum_temp = targetLum_filt(Params.EXTRAPOLATE_MARGIN/2 +1 : 1080 +Params.EXTRAPOLATE_MARGIN/2,...
                                    Params.EXTRAPOLATE_MARGIN/2 +1 : 1920 +Params.EXTRAPOLATE_MARGIN/2);
    targetLum = medfilt2(max(inputLum,targetLum_temp), [5 5]); 
    targetLum(targetLum==0) = targetLum_temp(targetLum==0);
    outIm = targetLum;
end

function [Ims, Params] = desiredBacklightVid(DbContents, Params, Ims, imNo)

    if Params.mod == 1
        imLum = readHdrThisIm(DbContents(imNo).name, Params, Ims);
        targetLum = desiredBacklightBase(Params, imLum);
        
        targetLumSm = imresize(targetLum, [1080/4 1920/4]);
        targetLumSm(targetLumSm < 0) = 0;
    elseif Params.mod == 2
        %if exist('hdrArr','var')
        if isfield(Params,'hdrArr')
            try
                comSet = intersect(Params.numArray(imNo-1,:),Params.numArray(imNo,:));
                toDelete = setdiff(Params.numArray(imNo-1,:),comSet);
                toDelIdx = find(Params.numArray(imNo-1,:)==toDelete);
                toAdd = setdiff(Params.numArray(imNo,:),comSet);
                toAddIdx = find(Params.numVector==toAdd);
                if ~isempty(toDelIdx)
                    Params.hdrArr(:,:,toDelIdx) = [];
                end
                if ~isempty(toAddIdx)
                    imset = toAddIdx;
                %if ~isempty(toAdd)
            ...
            imset = 1:Params.windowSize;
        end

        for ind = imset
            imLum = readHdrThisIm(DbContents(ind).name, Params, Ims);
%             disp([DbContents(ind).name ' has been read for ' DbContents(imNo).name]);
            targetLum = desiredBacklightBase(Params, imLum);

            % Store images in the window in an array of sizes of LED
            hdrTemp = imresize(targetLum, [37 60]);
            Params.hdrArr(:,:,ind) = hdrTemp;
        end

        % Use a filter that finds the maximum
        minIdx = find(Params.numVector==Params.numArray(imNo,1));
        maxIdx = find(Params.numVector==Params.numArray(imNo,end));
        hdrS = smoothMaxFilterN(Params.hdrArr(:,:,minIdx:maxIdx), Params.windowSize, 3,1.5);
        %if ~exist('hdrSM','var')
        if ~isfield(Params,'hdrSM')
            Params.hdrSM(:,:,1:Params.windowSize-1) = hdrS(:,:,1:end-1);
        else
            Params.hdrSM(:,:,minIdx:maxIdx-1) = max(Params.hdrSM(:,:,minIdx:maxIdx-1), hdrS(:,:,1:end-1));
        end
        Params.hdrSM(:,:,maxIdx) = hdrS(:,:,end);

        % Equate target lum
        targetLumSm = imresize(Params.hdrSM(:,:,imNo), [1080/4 1920/4]);
        targetLumSm(targetLumSm < 0) = 0;
    elseif Params.mod == 3
        if isfield(Params,'hdrArr')
            try
...
%             disp([DbContents(ind).name ' has been read for ' DbContents(imNo).name]);
            ...
            % Store images in the window in an array of sizes of LED
            hdrTemp = imresize(targetLum, [1080/16 1920/16]);
            Params.hdrArr(:,:,ind) = hdrTemp;
        end
        hdrS = mean(Params.hdrArr,3);
        
        % Equate target lum
        targetLumSm = imresize(hdrS, [1080/4 1920/4]);
        targetLumSm(targetLumSm < 0) = 0;
    end
...
function Ims = findLedValues(Params, Ims, imNo)
    ...
    if imNo == 1
        ...
        Ims.ledLumSm = conv2(leds, Params.led_psfSm, 'same');
        ...
        scale = imdilate(Ims.targetLumSm + Params.SAFETY_MARGIN, ones(25,25))./Ims.ledLumSm;
...
function Params = findWindowParams(Params, DbContents)
    
    % Find image numbers in image names
    % i.e. find "348" in "carousel_fireworks_03_000348.exr"
    [~, fName] = fileparts(DbContents(1).name);
    [~, lName] = fileparts(DbContents(end).name);
    pIdx = find( (fName == lName) == 0,1);
    Params.pStr = fName(1:pIdx-1);             % String prefix for image numbers
    Params.numLen = length(fName) - pIdx;      % Length of number, 3 for "348"

    for imNo = Params.fileIndices
        fileName = DbContents(imNo).name;
        Params.numVector(imNo) = str2double(fileName(pIdx:pIdx+Params.numLen));
    end

    % Find frames in the windowSize
    startingFrame = min(Params.numVector(:));
    endingFrame = max(Params.numVector(:));
    for imNo = Params.fileIndices
        % Two different window possibility
        if Params.windowTypeCentered
            %                   |-----------|    --> window
            % Centered window         x       -----> actual frame
            minInd = min(...
                        max(startingFrame,...
                            startingFrame+imNo-1-floor(Params.windowSize/2)),...
                        endingFrame-Params.windowSize+1);
        else
            %                   |-----------|    --> window
            % Forward window     x            -----> actual frame
            minInd = min( max(startingFrame,startingFrame+imNo-1),...
                          endingFrame-Params.windowSize+1);
        end
        numArray(imNo,:) = linspace(minInd,minInd+Params.windowSize-1,Params.windowSize);
    end
    Params.numArray = numArray;

end

function result = smoothMaxFilterN(data, windowSize, dimension, alpha)

    if ~exist('alpha','var'), alpha = 2.5; end
    % if ~isvector(data),  error('data should be a vector');  end
    if mod(windowSize, 2) == 0, error('windowSize should be odd');  end

    data = double(data);
    dS = size(data);            % Data size
    dL = size(data,dimension);  % Data length
    wHs = floor(windowSize/2);  % Window half size
    dataVec = cat(dimension, genZ(dS, wHs, dimension),... 
                     cat(dimension, data, genZ(dS, wHs, dimension)));
                 
    switch dimension
        case 1
            if numel(dS) == 3
                gaussArr = repmat(reshape(gausswin(windowSize,alpha)', [], 1), [1 dS(2:3)]);
            else
                gaussArr = repmat(reshape(gausswin(windowSize,alpha)', [], 1), [1 dS(2)]);
            end
        case 2
            if numel(dS) == 3
                gaussArr = repmat(reshape(gausswin(windowSize,alpha)', 1, []), [dS(1) 1 dS(3)]);
            else
                gaussArr = repmat(reshape(gausswin(windowSize,alpha)', 1, []), [dS(1) 1]);
            end
        case 3
            gaussArr = repmat(reshape(gausswin(windowSize,alpha)', 1, 1, []), [dS(1:2) 1]);
        otherwise
            error('This function supports dimensions up to 3');
    end

    switch dimension
        case 1
            if numel(dS) == 3
                tempVec = zeros(dL, dL+2*wHs, dS(2), dS(3));
            else
                tempVec = zeros(dL, dL+2*wHs, dS(2));
            end
        case 2 
            if numel(dS) == 3
                tempVec = zeros(dL, dS(1), dL+2*wHs, dS(3));
            else
                tempVec = zeros(dL, dS(1), dL+2*wHs);
            end
        case 3
            tempVec = zeros(dL, dS(1), dS(2), dL+2*wHs);
    end
    ...
    for ind = 1:dL
        switch dimension
            case 1
                if numel(dS) == 3
                    dataPart = repmat(dataVec(wHs+ind,:,:), [windowSize 1 1]);
                    tempVec(ind,:,:,:) = cat(dimension, genZ(dS,ind-1,dimension), cat(dimension, dataPart.*gaussArr, genZ(dS,idx-windowSize-ind+1,dimension)));
                else
                    dataPart = repmat(dataVec(wHs+ind,:), [windowSize 1]);
                    tempVec(ind,:,:) = cat(dimension, genZ(dS,ind-1,dimension), cat(dimension, dataPart.*gaussArr, genZ(dS,idx-windowSize-ind+1,dimension)));
                end
            case 2
                if numel(dS) == 3
                    dataPart = repmat(dataVec(:,wHs+ind,:), [1 windowSize 1]);
                    tempVec(ind,:,:,:) = cat(dimension, genZ(dS,ind-1,dimension), cat(dimension, dataPart.*gaussArr, genZ(dS,idx-windowSize-ind+1,dimension)));
                else
                    dataPart = repmat(dataVec(:,wHs+ind), [1 windowSize]);
                    tempVec(ind,:,:) = cat(dimension, genZ(dS,ind-1,dimension), cat(dimension, dataPart.*gaussArr, genZ(dS,idx-windowSize-ind+1,dimension)));
                end
            case 3
                dataPart = repmat(dataVec(:,:,wHs+ind), [1 1 windowSize]);
                tempVec(ind,:,:,:) = cat(dimension, genZ(dS,ind-1,dimension), cat(dimension, dataPart.*gaussArr, genZ(dS,idx-windowSize-ind+1,dimension)));
        end
    end
    ...
    switch dimension
        case 1
            if numel(dS) == 3
                result = sgolayfilt(reshape(max(tempVec(:, wHs+1:wHs+dL,:,:),[],1),dS(1),dS(2),dS(3)), 3, 7, [], dimension);
            else
                result = sgolayfilt(reshape(max(tempVec(:, wHs+1:wHs+dL,:),[],1),dS(1),dS(2)), 3, 7, [], dimension);
            end
        case 2 
            if numel(dS) == 3
                result = sgolayfilt(reshape(max(tempVec(:, :, wHs+1:wHs+dL,:),[],1),dS(1),dS(2),dS(3)), 3, 7, [], dimension);
...
function res = genZ(dS, size, dimension)
    switch dimension
        case 1
            if numel(dS) == 3
                zV = zeros(size, dS(2),dS(3));
            else
                zV = zeros(size, dS(2));
            end
        case 2
            if numel(dS) == 3
                zV = zeros(dS(1), size, dS(3));
            else
                zV = zeros(dS(1), size);
            end
        case 3
            zV = zeros(dS(1), dS(2), size);
    end
